...
create table TEEMO.time_subject (
    ...
    subject_id_fk_time_subject NUMBER(15),
    free_time_id_fk_time_subject NUMBER(15),
    constraint subject_id_fk_time_subject foreign key (subject_id_fk_time_subject) references TEEMO.subject (subject_id_pk),
    constraint free_time_id_fk_time_subject foreign key (free_time_id_fk_time_subject) references TEEMO.free_time (free_time_id_pk)
);

create table TEEMO.user_subject (
    dificulty NUMBER(15) NOT NULL,
    user_id_fk_user_subject NOT NULL,
    subject_id_fk_user_subject NOT NULL,
    constraint user_id_fk_user_subject foreign key (user_id_fk_user_subject) references TEEMO.users (user_id_pk),
    constraint subject_id_fk_user_subject foreign key (subject_id_fk_user_subject) references TEEMO.subject (subject_id_pk)
);
